...
class Estrategia
{
		...
		else
		{
		    ...
		    $constanteDeudaNot = "SELECT Persona.Rut,Deuda.Monto_Mora FROM Persona,Deuda WHERE NOT Persona.Rut IN ";
				...
				if($or_count>0)
				{
					...
					if($rel == 0)
					{
						$subQuery = "(SELECT Rut FROM $tablas WHERE $or_query)  AND FIND_IN_SET('$this->cedente',Id_Cedente)";
					...
					{
						$subQuery = "(SELECT Rut FROM $tablas WHERE $or_query AND Id_Cedente = $this->cedente)  AND FIND_IN_SET('$this->cedente',Id_Cedente)";
				...
				else
				{
					...
					if($rel == 0)
					{
						$subQuery = "(SELECT Rut FROM $tablas WHERE $columnas $this->logica $valor ) AND FIND_IN_SET('$this->cedente',Id_Cedente)";
					...
					{
						$subQuery = "(SELECT Rut FROM $tablas WHERE $columnas $this->logica $valor AND Id_Cedente = $this->cedente) AND FIND_IN_SET('$this->cedente',Id_Cedente)";
					...
					{
						$subQuery = "(SELECT Rut FROM $tablas WHERE $columnas $this->logica $valor AND FIND_IN_SET('$this->cedente',Id_Cedente)) AND FIND_IN_SET('$this->cedente',Id_Cedente)";
	...
	public function mostrarRelacionDos()
	{
		...
		if($or_count > 0)
		{
			...
			if($rel == 0)
			{
				$columnas3 = "(SELECT Rut FROM $tablas WHERE $or_query) AND FIND_IN_SET('$this->cedente',Id_Cedente)";	
			}
			else if($rel == 1)
			{
				$columnas3 = "(SELECT Rut FROM $tablas WHERE $or_query AND Id_Cedente = $this->cedente)  AND FIND_IN_SET('$this->cedente',Id_Cedente)";
		...
		else
		{
			if($rel == 0)
			{
				$columnas3 = "(SELECT Rut FROM $tablas WHERE $columnasQuery $this->logica $valor ) AND FIND_IN_SET('$this->cedente',Id_Cedente)";

			}	
			else if($rel == 1)
			{
				$columnas3 = "(SELECT Rut FROM $tablas WHERE $columnasQuery $this->logica $valor AND Id_Cedente = $this->cedente) AND FIND_IN_SET('$this->cedente',Id_Cedente)";

			}
			else
			{
				$columnas3 = "(SELECT Rut FROM $tablas WHERE $columnasQuery $this->logica $valor AND FIND_IN_SET('$this->cedente',Id_Cedente)) AND FIND_IN_SET('$this->cedente',Id_Cedente)";
			}	
		...
		if($r == 1)
		{
			$condicion=" AND NOT Rut IN ".$columnas3;
		}
		else
		{
			$condicion=" AND Rut IN ".$columnas3;
		}	
		...
		while($row=mysql_fetch_array($querya))
		{
		  ...
		  else
		  {
			...
			$condicion=" AND ".$row[1]." Rut IN ".$row[2];
			array_push($array_central, $condicion);
