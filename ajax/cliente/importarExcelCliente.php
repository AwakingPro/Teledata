	...
	foreach ($objPHPExcel->getWorksheetIterator() as $index => $worksheet) {
		...
		if($index == 1){
			for ($row = 2; $row <= $highestRow; ++ $row) {
				...
				$query = " INSERT INTO servicios (Rut, Grupo, TipoFactura, Valor, Descuento, IdServicio, TiepoFacturacion, Codigo, Descripcion, TipoMoneda, IdUsuarioSession, Alias, Estatus, FechaInstalacion, InstaladoPor, Comentario, UsuarioPppoe, Direccion, Latitud, Longitud, Referencia, Contacto, Fono, PosibleEstacion, Equipamiento, SenalTeorica, IdUsuarioAsignado, SenalFinal, EstacionFinal, EstatusFacturacion, CostoInstalacion, CostoInstalacionTipoMoneda, CostoInstalacionDescuento, FacturarSinInstalacion, FechaComprometidaInstalacion) VALUES ('".$Rut."', '1', 'Boleta', '".$Valor."', '0', '".$IdServicio."', 'Mensual', '".$Codigo."', '".$Descripcion."', '".$tipoMoneda."', '".$idUsuario."', '".$Codigo."', '0', '1970-01-01', '', '', '', '', '', '', '', '', '', '', '', '', '0', '', '', '0', '', '".$tipoMoneda."', '0', '0', '1970-01-01')";
				$id = $run->insert($query);
				if(!$id){
					echo 'Error 1';
				}
