...
$(document).ready(function() {
    ...
    /*$("#GroupRecords").click(function(){
        if(!GroupRecordsFlag){
            CustomAlert('Debe seleccionar almenos una evaluación');
            return false;
        }
        var Template = $("#RecordGroupsTemplate").html();
        var ObjectMe = $(this);

        bootbox.dialog({
            title: "AGRUPACIÓN DE GRABACIONES",
            message: Template,
            buttons: {
                confirm: {
                    label: "Save",
                        ...
                        if(Observation_aspectosF != ""){
                            if(Observation_aspectosC != ""){
                                if(Observation_comprimisoE != ""){
                                    ...
                                    CustomAlert('Debe agregar una observacion en el campo "Compromiso del Ejecutivo"');
                                    return false;
                                }
                            }else{
                                CustomAlert('Debe agregar una observacion en el campo "Aspectos a Corregir"');
                                return false;
                            }
                        }else{
                            CustomAlert('Debe agregar una observacion en el campo "Aspectos a Fortalecer"');
                            return false;
                        }
                    }
                }
    ...
    $("body").on("change","#Records .groupSelecter",function(){
            if($(this).prop('checked')){
                GroupRecordsFlag = true;
                $("#GroupRecords").removeClass("ElementInvisible");
            }else{
                var Cont = 0;
                $("#Records .groupSelecter").each(function(){
                    if($(this).prop("checked")){
                        Cont++;
                    }
                });
                if(Cont <= 0){
                    GroupRecordsFlag = false;
                    $("#GroupRecords").addClass("ElementInvisible");
                }
            }
        var ID = $(this).attr("id");
        if($(this).prop('checked')){
            RecordGroups[ID] = true;
        }else{
    ...
    $("body").on("change","select[name='Ejecutivo']",function(){
        ...
        if(Val != ""){
            ...
            if(HizoCierre(idMandante,idCartera)){
                ...
                UpdateRecords(idMandante,idCartera);
            }
        }
    });
    ...
    $("#FinDeProceso").click(function(){
        ...
        bootbox.dialog({
            title: "CIERRE DE PROCESO DE MES DE LA CARTERA '" + GlobalData.nombre_cedente.toUpperCase() + "'",
            message: Template,
            buttons: {
                success: {
                    label: "Guardar",
                    ...
                    callback: function() {
                        ...
                        if(Observation_aspectosF != ""){
                            if(Observation_aspectosC != ""){
                                if(Observation_comprimisoE != ""){
                                    CierreDeProceso(idMandante,idCartera,Observation_aspectosF,Observation_aspectosC,Observation_comprimisoE);
                                }else{
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                if(GlobalData.isEjecutivo){
                    ...
                    UpdateRecords(GlobalData.id_mandante,GlobalData.id_cedente);
                    $("#FinDeProceso").removeClass("ElementInvisible");
                }else{
                    PreloadRecordTable();
                }
    ...
    function UpdateRecords(idMandante,idCartera){
        $.ajax({
            type: "POST",
    ...
    function CierreDeProceso(idMandante,idCartera,Observation_aspectosF,Observation_aspectosC,Observation_comprimisoE){
        $.ajax({
            type: "POST",
