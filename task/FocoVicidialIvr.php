<?php 
//ASTERISK VICIDIAL
$ConViciDial=mysql_connect('192.168.1.220','root','m9a7r5s3'); 
      ...
      // 	$FechaCompromisoP= $row[3];
       ...
       $QueryGrabacion = mysql_query("SELECT filename,location FROM recording_log WHERE lead_id = $LeadId",$ConViciDial);
        ...
        while($row = mysql_fetch_array($QueryLead)){
            ...
            $StatusName = "";
            $IdTipoGestion = "";
            $QueryStatus = mysql_query("SELECT status_name FROM vicidial_statuses WHERE status = '$Status'",$ConViciDial);
...
*/      /*    if($PhoneNumber==''){
 	...
 	echo $Insert = "INSERT INTO gestion_ult_trimestre_ivr(rut_cliente,fechahora,observacion,fecha_gestion,hora_gestion,lista,cedente,fono_discado,duracion,nombre_ejecutivo,nombre_grabacion,origen,Id_TipoGestion,status,status_name,fec_compromiso,Ponderacion,url_grabacion) VALUES ('$Rut','$CallDate','$Observacion','$FechaGestion','$HoraGestion','$Lista','$Campana','$PhoneNumber','$Duracion','$Agente','$Grabacion','777','1','$Status','$StatusName','$FechaCompromiso','1','$Location')";
